...
class GuildHandler implements CommandExecutor
{
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool
    {
        ...
        if ($args[0] === "생성") {
            if (count($args) != 3) {
                $player->sendMessage("/길드 생성 <길드이름> <태그>을 사용하세요!");
                return true;
            }
            if (strlen($args[2]) != 4) {
                $player->sendMessage("길드 태그는 4글자여야 합니다!");
                return true;
            }
            if (isset($this->plugin->config["Guilds"])) {
                foreach ($this->plugin->config["Guilds"] as $guilds => $value) {
                    if ($args[2] === $this->plugin->config["Guilds"][$guilds]["Tag"]) {
                        $player->sendMessage("이 길드 태그는 이미 존재합니다!");
                        return true;
                    }
                }
            }
            if (!$player->hasPermission("guild.create")) {
                $player->sendMessage("이 명령을 사용할 권한이 없습니다!");
                return true;
            }
            if (isset($this->plugin->config[$player->getName()])) {
                $player->sendMessage("이미 가입된 길드가 있습니다! 새로운 길드를 만들 수 없습니다!");
                return true;
            }
            if (isset($this->plugin->config[str_replace("_", " ", $args[1])])) {
                $player->sendMessage(str_replace("_", " ", $args[1]) . " 이름을 가진 길드가 이미 존재합니다!");
                return true;
            }
            ...
            $this->plugin->config["Guilds"][str_replace("_", " ", $args[1])]["Ranks"]["Leader"]["Title"] = "§4길드 마스터";
            ...
            $this->plugin->config["Guilds"][str_replace("_", " ", $args[1])]["Ranks"]["Newbies"]["Title"] = "§2초보자";
            $this->plugin->config["Guilds"][str_replace("_", " ", $args[1])]["DefTerm"]["Default"] = "Newbies";
            $this->plugin->saveConfig();
            $player->sendMessage("축하합니다 " . $player->getName() . " 당신은 이제 새로 만들어진 길드 " . str_replace("_", " ", $args[1]) . "의 리더입니다.");
            return true;
        } else if ($args[0] === "초대") {
            if (count($args) != 2) {
                $player->sendMessage("잘못된 사용법 입니다, /길드 초대 <유저>을 사용하세요.");
                return true;
            }
            ...
            if (!$this->plugin->config["Guilds"][$guildn]["Ranks"][$grank]["Invite"]) {
                $player->sendMessage("이 길드에 사람들을 초대 할 권한이 없습니다!");
                return true;
            }
            if (isset($this->plugin->config[$pi])) {
                $player->sendMessage($pi . "님은 이미 길드에 있습니다!");
                return true;
            }
            if (isset($this->plugin->config["Pending"][$pi])) {
                $player->sendMessage("이 플레이어는 이미" . $this->plugin->config["Pending"][$pi]["Guild"] . "에서 대기중인 길드 초대를 받았습니다!");
                return true;
            }
            $this->plugin->config["Pending"][$pi]["Guild"] = $guildn;
            $this->plugin->saveConfig();
            $player->sendMessage($pi . "님을 " . $guildn . "에 초대하였습니다!");
            $p->sendMessage($guildn . "에서 대기중인 길드 초대가 있습니다! 이 길드에 가입하려면 </길드 수락>를 입력하십시오! 또는 </길드 거절>를 입력하여 거절하십시오!");
            return true;
        } else if ($args[0] === "수락") {
            if (!isset($this->plugin->config["Pending"][$player->getName()])) {
                $player->sendMessage("대기중인 길드 초대가 없습니다!");
                return true;
            }
            $guildn2 = $this->plugin->config["Pending"][$player->getName()]["Guild"];
            if (!isset($this->plugin->config["Guilds"][$guildn2])) {
                $player->sendMessage("이 길드가 더 이상 존재하지 않습니다!");
            ...
            foreach ($this->plugin->config["Guilds"][$guildn2]["Players"] as $key => $value) {
                if (Server::getInstance()->getPlayer($key) != null) {
                    if (Server::getInstance()->getPlayer($key)->getName() == $player->getName()) {
                        $player->sendMessage($guildn2 . "에 가입하였습니다!!!");
                    } else {
                        $p2 = Server::getInstance()->getPlayer($key);
                        $p2->sendMessage("§3" . $player->getName() . "§2님이 길드에 가입하였습니다!");
                    }
                }
            }
            return true;
            ...
            if (Server::getInstance()->getPlayer($lead) != null) {
                ...
                $gleader->sendMessage($player->getName() . "님이 길드 초대를 거절했습니다!");
            }
            $player->sendMessage($guildn2 . " 가입을 거절했습니다!");
            $this->plugin->config["Pending"][$player->getName()] = null;
            $this->plugin->saveConfig();
            return true;
            ...
            if (Server::getInstance()->getPlayer($args[1]) != null) {
                ...
                $p3->sendMessage($guildn2 . "에서 추방되었습니다!");
                $p3->setDisplayName($p3->getName());
            }
            $player->sendMessage($guildn2."에서 " . $args[1] . "님을 추방하였습니다!");
            $this->plugin->saveConfig();
            return true;
        } else if ($args[0] === "해산") {
            $guildn2 = $this->plugin->config[$player->getName()]["Guild"]["Name"];
            ...
            if (!$this->plugin->config["Guilds"][$guildn2]["Ranks"][$grank2]["Disband"]) {
                $player->sendMessage("길드를 해산 할 권한이 없습니다!");
                return true;
            }
            foreach ($this->plugin->config["Guilds"][$guildn2]["Players"] as $key => $value) {
                ...
                if (Server::getInstance()->getPlayer($key) != null) {
                    if (Server::getInstance()->getPlayer($key)->getName() == $player->getName()) {
                        $player->sendMessage("§4" . $guildn2 . "를 해산하였습니다!!!");
                        $player->setDisplayName($player->getName());
                    } else {
                        $p2 = Server::getInstance()->getPlayer($key);
                        $p2->setDisplayName($p2->getName());
                        $p2->sendMessage("§3" . $player->getName() . "§4님이 길드를 해산하였습니다!");
                    }
                }
            }
            ...
            foreach ($this->plugin->config["Guilds"][$guildn2]["Players"] as $key2 => $value2) {
                if (Server::getInstance()->getPlayer($key2)->getName() == $player->getName()) {
                    $player->sendMessage("§4" . $guildn2 . "를 탈퇴하였습니다!!!");
                } else {
                    if (Server::getInstance()->getPlayer($key2) == null) {
                        continue;
                    }
                    $p3 = Server::getInstance()->getPlayer($key2);
                    $p3->sendMessage("§3" . $player->getName() . "§4님이 길드를 탈퇴하였습니다!");
                }
            }
            ...
            if (!$args[0] === "목록") {
                if ($args[0] === "TP") {
                    if (!$player->isOp()) {
                        $player->sendMessage("OP만 길드 순간이동 작동을 변경할 수 있습니다!");
                        return true;
                    }
                    if (count($args) != 2) {
                        $player->sendMessage("잘못된 사용법 입니다! /길드 tp on 또는 /길드 tp off을 사용하세요!");
                        return true;
                    }
                    if ($args[1] === "on") {
                        $this->plugin->config["TP"] = true;
                        $player->sendMessage("길드원 순간이동이 서버에서 허용됩니다!");
                        return true;
                    }
                    if ($args[1] === "off") {
                        $this->plugin->config["TP"] = false;
                        $player->sendMessage("길드원 순간이동은 더 이상 서버에서 허용되지 않습니다!");
                        return true;
                    }
                }
                return false;
            ...
            if (count($args) != 1) {
                $player->sendMessage("잘못된 사용법 입니다! /길드 목록을 사용하세요!");
                return true;
            }
            $guildn2 = $this->plugin->config[$player->getName()]["Guild"]["Name"];
            if (!isset($this->plugin->config[$player->getName()])) {

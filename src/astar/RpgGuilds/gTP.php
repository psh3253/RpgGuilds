...
class gTP implements CommandExecutor
{
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool
    {
        ...
        if (!$player->hasPermission("guild.teleport")) {
            $player->sendMessage("§d[ §f길드 §d] §f이 서버에서 유저들을 순간이동할 권한이 없습니다!");
            return true;
        }
        if (count($args) == 0) {
            return false;
        ...
        if (!$this->plugin->config["TP"]) {
            $player->sendMessage("§d[ §f길드 §d] §f이 서버에서 길드원 순간이동이 비활성화 되어있습니다!");
            return true;
        }
        ...
        if (!isset($this->plugin->config[$pi])) {
            $player->sendMessage("§d[ §f길드 §d] §f해당 유저는 아직 길드가 없습니다!");
            return true;
        }
        ...
        if (!$this->plugin->config["Guilds"][$guildn]["Ranks"][$grank]["TP"]) {
            $player->sendMessage("§d[ §f길드 §d] §f이 길드에서 유저들을 순간이동할 권한이 없습니다!");
            return true;
        }
        if ($guildn !== $guildn2) {
            $player->sendMessage("§d[ §f길드 §d] §f해당 유저는 당신의 길드에 없습니다, 자신의 길드원만 순간이동할 수 있습니다!");
            return true;
        }
